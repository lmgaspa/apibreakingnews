const { createService, findAllService, countNews, topNewsService, findByIdService,
    searchByTitleService, byUserService,
    updateService, likeNewsService,
    deleteLikeNewsService, addCommentService, deleteCommentService } = require('../services/news.service.js')
    ...
    try {
        ...
        return res.send({
            news: {
                id: news._id,
                title: news.title,
                text: news.banner,  // Talvez isso devesse ser 'text' em vez de 'banner'?
                banner: news.banner,
                likes: news.likes,
                comments: news.comments,
                name: news.user.name,
...
const update = async (req, res) => {
    try {
        ...
        if (!news || news.user._id != req.userId) {
            return res.status(400).send({
...
const deleteComment = async (req, res) => {
    try {
        ...
        // Verificar se commentDeleted ou commentDeleted.comments é undefined
        if (!commentDeleted || !commentDeleted.comments) {
        ...
        if (commentFinder.userId !== userId) {
            return res.status(400).send({ message: "You can't delete this commentary" });
        }
...
module.exports = { create, findAll, topNews, findById, searchByTitle, byUser, update, erase, likeNews, addComment, deleteComment };
